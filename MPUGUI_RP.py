...
def run():
    ...
    pygame.display.set_caption('MPU ROLL & PITCH 3D MAPPING')
    resize(*SCREEN_SIZE)
    init()
    ...
    while counter <= limit - 1:
        ...
        for event in pygame.event.get():
            ...
            if event.type == MOUSEBUTTONUP:
                print ('TIME: {:.4f} ROLL: {:.4f} PITCH: {:.4f} YAW: {:.4f}'.format(float(timer),float(x_angle),-float(y_angle)))

        timer = time[counter]
        x_angle = xdegree[counter]
        y_angle = ydegree[counter]
