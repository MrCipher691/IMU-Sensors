...
df_data = pd.read_csv(r'pureaccel.csv')
...
def run():
    ...
    pygame.display.set_caption('MPU ROLL, PITCH & YAW 3D MAPPING')
    resize(*SCREEN_SIZE)
    init()
    ...
    while counter <= limit - 1:
        ...
        for event in pygame.event.get():
            ...
            if event.type == MOUSEBUTTONUP:
                print ('TIME: {:.4f} ROLL: {:.4f} PITCH: {:.4f} YAW: {:.4f}'.format(float(timer),-float(roll),float(pitch),-float(yaw)))

        timer = time[counter]
        pitch = ydegree[counter]
        roll = xdegree[counter]
        yaw = zdegree[counter]
